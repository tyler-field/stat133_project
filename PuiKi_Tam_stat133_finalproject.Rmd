...
labels2 <- paste(labels, piepercent)
labels2 <- paste(labels2, "%", sep="")

png(file= "DelayPie.jpg")

pie(pie_number, labels= labels2, main = "Delay Causes in January 2017")

dev.off()
```

```{r}
bar_numbers <- c(countCarrierD, countWeatherD, countNasD, countSecurityD, countAircraftD)
bar_causes <- c("Carrier", "Weather", "Nas", "Security", "Late Aircraft")
BarData <- data.frame(Delay.Causes= bar_causes, Number= bar_numbers)

BarData = BarData[with(BarData, order(-bar_numbers)), ]

BarData

png(file= "DelayBarplot.jpg")

barplot(BarData$Number, main = "Delay Causes in January 2017", col="grey50", names.arg = BarData$Delay.Causes, cex.names =0.85, las=1)
